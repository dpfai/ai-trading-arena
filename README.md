...
That wrapper updates upstream inputs first, then calls `scripts/build_trading_arena_data.py`. It runs `quant-learning/daily_runner.py` so QuantAI does not stall, ensures a `stock_analysis_YYYY-MM-DD.json` file exists for AI Analyst, and validates the final JSON source coverage.
